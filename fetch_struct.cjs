...
https.get(urlStruct, { headers: { 'Accept': 'application/vnd.sdmx.data+json; charset=utf-8; version=2.0' } }, (resp) => {
  ...
  resp.on('end', () => { 
      try {
          ...
          if (indicator) {
              console.log("Struct Indicators:", indicator.values.map(v => v.name));
          } else {
              console.log("Struct Dims:", dim.map(d=>d.id));
              let isM = dim.find(d => d.id === 'MEASURE');
              if(isM) console.log(isM.values.map(v=>v.name));
          }
      } catch(e) {}
  });
});
